...
> "Empezando por el acceso, implementé un **Login Híbrido**, permitiendo entrar con correo o con nombre de usuario. Pero el valor añadido aquí es la seguridad. Si inicio sesión... *(inicias sesión y entras al Home)*... el sistema me asigna un UUID único mediante Firebase. Si yo le diese mi cuenta a un amigo y él entrara ahora mismo, el sistema detectaría el cambio a través de un WebSocket nativo (`onSnapshot`) y me expulsaría al instante. Cero cuentas compartidas."
...
> "Como esto es una red social deportiva, cada usuario puede construir su propio equipo soñado. He implementado validaciones estrictas: la interfaz bloquea cualquier intento de colocar a un Pivot (C) en la posición de un Base (PG) cruzando el mapa de mi frontend con el JSON que devuelve la API.
